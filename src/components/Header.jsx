...
export default function Header() {
  ...
  return (
    <header style={{ position: 'absolute', top: 16, left: 0, right: 0, zIndex: 9999, backgroundColor: 'transparent' }} className="md:!top-4 !top-4">
      <div className="w-full px-6 py-4 flex items-center justify-between relative">
        <div className="flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-gray-800 font-menu normal-case flex-shrink-0">
            ...
            >
              <Link className="hover:text-primary transition-colors whitespace-nowrap flex items-center gap-1" to="/productos" onClick={() => setProductsDropdownOpen(false)}>
                Productos
              ...
              {productsDropdownOpen && navCategories.length > 0 && (
                <div className="absolute top-full left-0 pt-2 z-50">
                  <div className="bg-white border border-gray-100 rounded-lg shadow-lg py-2 min-w-[180px]">
                    <Link className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary transition-colors whitespace-nowrap" to="/productos" onClick={() => setProductsDropdownOpen(false)}>Todos</Link>
                    {navCategories.map((cat) => (
                      <Link key={cat.id} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary transition-colors whitespace-nowrap" to={`/productos?categoria=${cat.slug}`} onClick={() => setProductsDropdownOpen(false)}>{cat.name}</Link>
                    ))}
                  </div>
                </div>
              )}
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center">
          <Link to="/" className="flex items-center"><img src="/logo-v2.png" alt="Astra Boutique" className="h-9 md:h-16" /></Link>
        </div>
        <div className="flex items-center gap-5 justify-end flex-shrink-0">
          <button className="hover:text-primary transition-colors flex items-center" onClick={() => setSearchOpen(!searchOpen)}><Search size={20} strokeWidth={0.75} /></button>
          <button className="relative hover:text-primary transition-colors flex" onClick={() => setIsCartOpen(true)}>
            ...
            {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-primary text-white text-[8px] w-4 h-4 flex items-center justify-center rounded-full">{cartCount}</span>}
          </button>
          <Link to="/cuenta" className="hover:text-primary transition-colors flex"><User size={20} strokeWidth={0.75} /></Link>
        </div>
      </div>

      {mobileMenuOpen && (
        <>
          <div className="fixed inset-0 bg-black/20 z-[9998] md:hidden" onClick={() => setMobileMenuOpen(false)} />
          <div className="fixed top-0 left-0 h-full w-full z-[9999] md:hidden animate-slideIn" style={{ backgroundColor: '#F8F5F1' }}>
            <div className="flex flex-col h-full">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <Link to="/" className="flex items-center" onClick={() => setMobileMenuOpen(false)}><img src="/logo-v2.png" alt="Astra Boutique" className="h-9 md:h-16" /></Link>
                <button className="hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}><X size={16} strokeWidth={0.75} /></button>
              </div>
              <div className="flex-1 overflow-y-auto">
                <nav className="flex flex-col px-6 pt-8 gap-1">
                  <Link className="text-sm font-medium text-gray-800 font-menu hover:text-primary transition-colors normal-case py-2" to="/productos?sort=newest" onClick={() => setMobileMenuOpen(false)}>Nuevos</Link>
                  <button className="flex items-center justify-between py-2 text-sm font-medium text-gray-800 font-menu hover:text-primary transition-all normal-case" onClick={() => setMobileProductsOpen(!mobileProductsOpen)}>
                    Productos
                  ...
                  <div className={`overflow-hidden transition-all duration-300 ${mobileProductsOpen ? 'max-h-60 pb-2' : 'max-h-0'}`}>
                    <div className="flex flex-col gap-1 pl-3">
                      <Link className="text-sm text-gray-600 font-menu hover:text-primary transition-colors normal-case py-1.5" to="/productos" onClick={() => { setMobileMenuOpen(false); setMobileProductsOpen(false) }}>Todos</Link>
                      {navCategories.map((cat) => (
                        <Link key={cat.id} className="text-sm text-gray-600 font-menu hover:text-primary transition-colors normal-case py-1.5" to={`/productos?categoria=${cat.slug}`} onClick={() => { setMobileMenuOpen(false); setMobileProductsOpen(false) }}>{cat.name}</Link>
                      ))}
                    </div>
                  </div>
                ...
                <div className="px-6 mt-10 pt-6 border-t border-gray-100">
                  ...
                  <div className="space-y-3">
                    <a href="https://wa.me/573155614103" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-gray-600 hover:text-primary transition-colors" onClick={() => setMobileMenuOpen(false)}>
                    ...
                    <a href="mailto:hello@astraboutique.store" className="flex items-center gap-3 text-sm text-gray-600 hover:text-primary transition-colors">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-[16px] h-[16px]"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg>
                      hello@astraboutique.store
                    </a>
                    <p className="flex items-center gap-3 text-sm text-gray-600">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-[16px] h-[16px]"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
              ...
              <div className="px-6 py-6 border-t border-gray-100">
                <a href="https://wa.me/573155614103" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full bg-[#25D366] text-white rounded-full py-3 text-sm font-medium transition-opacity hover:opacity-90" onClick={() => setMobileMenuOpen(false)}>
      ...
      {searchOpen && (
        <div className="fixed top-0 left-0 h-full w-full z-[9999]" style={{ backgroundColor: '#F8F5F1' }}>
          <div className="flex flex-col h-full">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <Link to="/" className="flex items-center" onClick={() => setSearchOpen(false)}><img src="/logo-v2.png" alt="Astra Boutique" className="h-9 md:h-16" /></Link>
              <button className="hover:text-primary transition-colors" onClick={() => setSearchOpen(false)}><X size={16} strokeWidth={0.75} /></button>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-8">
              <div className="relative flex items-center mb-12">
                <Search className="absolute left-4 text-gray-400" size={20} strokeWidth={0.75} />
                <input className="w-full bg-white border border-gray-200 rounded-full py-4 pl-12 pr-4 text-base font-menu focus:ring-1 focus:ring-primary focus:outline-none" placeholder="Buscar productos..." type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} autoFocus />
              </div>
                  ...
                  {searchResults.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {searchResults.map((product) => (
                        <Link key={product.id} to={`/producto/${product.slug || product.id}`} onClick={() => setSearchOpen(false)} className="group cursor-pointer">
              ...
              {searchQuery.trim().length < 2 && randomProducts.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-gray-800 font-menu mb-6">Búsqueda rápida</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="fixed inset-0 bg-black/20 z-[9998]" style={{ marginTop: '-100vh' }} onClick={() => setSearchOpen(false)} />
        </div>
      )}
    </header>
  )
