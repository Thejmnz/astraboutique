...
export default function BioLinksPage() {
  ...
  const fetchData = async () => {
    const [linksRes, socialsRes] = await Promise.all([
      supabase.from('bio_links').select('*').eq('is_active', true).order('position', { ascending: true }),
      supabase.from('bio_socials').select('*').eq('is_active', true),
    ])

    if (!linksRes.error) setLinks(linksRes.data || [])
    if (!socialsRes.error) setSocials(socialsRes.data || [])
    setLoading(false)
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center">
        <div className="w-5 h-5 border-2 border-gray-200 border-t-[#251E1A] rounded-full animate-spin" />
      </div>
    )
  }
      ...
      <div className="w-full max-w-sm">
        ...
        {socials.length > 0 && (
          <div className="flex items-center justify-center gap-5 mb-8">
            {socials.map((social) => {
              const svgString = SOCIAL_ICONS[social.platform]
              ...
              return (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-[#251E1A] hover:text-white hover:border-[#251E1A] transition-all duration-200"
        ...
        {links.length === 0 ? (
          <p className="text-center text-gray-400 text-sm font-menu">Aun no hay links disponibles</p>
        ) : (
          <div className="space-y-3">
            {links.map((link) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block w-full py-3.5 px-5 rounded-xl bg-white border border-gray-200 text-[#1A1A1A] text-sm font-menu text-center hover:bg-[#251E1A] hover:text-white hover:border-[#251E1A] transition-all duration-200"
              >
                {link.title}
              </a>
            ))}
        ...
        <p className="text-center text-gray-300 text-xs mt-12 font-menu">
          astraboutique.store
        </p>
      </div>
    </div>
