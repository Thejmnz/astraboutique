...
const PLATFORMS = [
  { id: 'instagram', label: 'Instagram', svg: SOCIAL_SVGS.instagram },
  { id: 'whatsapp', label: 'WhatsApp', svg: SOCIAL_SVGS.whatsapp },
  { id: 'tiktok', label: 'TikTok', svg: SOCIAL_SVGS.tiktok },
]
...
export default function BioLinksAdmin() {
  ...
  const confirmDelete = async () => {
    const { error } = await supabase.from('bio_links').delete().eq('id', deleteModal.id)
    if (!error) {
      const reordered = links.filter(l => l.id !== deleteModal.id).map((l, i) => ({ ...l, position: i + 1 }))
      ...
      <div className="p-8">
        ...
        <div className="bg-white rounded-lg border border-gray-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {PLATFORMS.map(({ id, label, svg }) => (
                <div key={id} className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-gray-50 flex items-center justify-center text-gray-500 flex-shrink-0" dangerouslySetInnerHTML={{ __html: svg }}>
                  </div>
                  <input
                    type="text"
